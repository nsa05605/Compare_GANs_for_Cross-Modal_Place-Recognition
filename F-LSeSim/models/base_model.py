...
class Vgg16(torch.nn.Module):
    def __init__(self, requires_grad=False):
        ...
        self.vgg_pretrained_features = visionmodels.vgg16(pretrained=True).features
        # visionmodels.resnet50(pretrained=True).features
...
## 여기에서 indices 부분을 수정해서 multi-layer PerceptualLoss를 만들기
class PerceptualLossVgg16(nn.Module):
    def __init__(self, vgg=None,  gpu_ids=[0,1,2,3], weights=None, indices=None, normalize=None):
    ...
    def forward(self, x, y):
        ...
        loss = 0
        # print("vgg : {}".format(len(x_vgg)))
        for i in range(len(x_vgg)):
            loss += self.weights[i] * self.criterion(x_vgg[i], y_vgg[i].detach())
            # print("Perceptual_Loss {} : {}".format(i, loss))
        return loss